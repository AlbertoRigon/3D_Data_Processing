...
#pragma omp parallel for
  ...
  // aggregated matching cost for a pixel
  ...
  // aggregated precomputed matching cost for a pixel
...
#pragma omp parallel for
  ...
  void PatchMatch::spatial_propagation(int x, int y, int cpv, int iter)
  {
    ...
    // vector in which we store the disparities of the 2 neighbours
    std::vector<float> disparities;

    // vector in which we store the related costs of (x, y) using 
    // the disparities above
    std::vector<float> costs;

    // get current disparity at location(x, y)
    float current_disparity = disps_[cpv](y, x);
    
    // get current matching cost at location (x, y)
    float current_cost = costs_[cpv](y, x);

    // check if iter is even or odd
    if (iter % 2 == 0)
    {
      if (inside(x - 1, y, 0, 0, cols_, rows_))
      {
        // get current disparity at location(x - 1, y) (left neighbor)
        float disp = disps_[cpv](y, x - 1);
        // compute cost with new disparity
        float cost = precomputed_disp_match_cost(disp, x, y, cpv);

        // store the disparity and related cost into the vectors
        disparities.push_back(disp);
        costs.push_back(cost);
      }

      if (inside(x, y - 1, 0, 0, cols_, rows_))
      {
        // get current disparity at location(x, y - 1) (lower neighbor)
        float disp = disps_[cpv](y - 1, x);
        // compute cost with new disparity
        float cost = precomputed_disp_match_cost(disp, x, y, cpv);
    ...
    else
    {
      if (inside(x + 1, y, 0, 0, cols_, rows_))
      {
        // get current disparity at location(x + 1, y) (right neighbor)
        float disp = disps_[cpv](y, x + 1);
        // compute cost with new disparity
        float cost = precomputed_disp_match_cost(disp, x, y, cpv);
      ...
      if (inside(x, y + 1, 0, 0, cols_, rows_))
      {
        // get current disparity at location(x, y + 1) (bottom neighbor)
        float disp = disps_[cpv](y + 1, x);
        // compute cost with new disparity
        float cost = precomputed_disp_match_cost(disp, x, y, cpv);
    ...
    // check if the matching costs decrease and update the cost and
    // disparity if so  
    for (int i = 0; i < costs.size(); i++) 
      if (costs[i] < current_cost)
      {
        // update disparity and cost of location (x, y)
        // if we find a better cost
        disps_[cpv].at<float>(y, x) = disparities[i];
        costs_[cpv].at<float>(y, x) = costs[i];     
        
        // update current cost for the next iteration
        current_cost = costs_[cpv].at<float>(y, x); 
      }
  } 
  ...
  void PatchMatch::view_propagation(int x, int y, int cpv)
  {
    ...
    // get current matching cost at location (x, y)
    float current_cost = costs_[cpv].at<float>(y, x);

    // vector in which we store the disparities of pixels
    // having pixel (x, y) as match in the other view
    std::vector<float> disparities;

    if (cpv == 0)
    {
      // we check the right image only on the same row of the given pixel,
      // since disparity is defined along x; for cpv = 0, matches 
      // for pixel (x,y) can only be found at left of column x
      for (int i = 0; i <= x; ++i)
      {
        float current_disp = disps_[1].at<float>(y, i);
        if (i + current_disp == x && inside(i + current_disp, y, 0, 0, cols_, rows_))
          disparities.push_back(current_disp);
      }
    }
    else
    {
      // we check the left image only on the same row of the given pixel,
      // since disparity is defined along x; for cpv = 1, matches 
      // for pixel (x,y) can only be found at right of column x
      for (int i = x; i < cols_; ++i)
      {
        float current_disp = disps_[0].at<float>(y, i);
        if (i - current_disp == x && inside(i - current_disp, y, 0, 0, cols_, rows_))
          disparities.push_back(current_disp);
      }
    }
    ...
    {
      // compute cost of pixel (x, y) using the disparities
      // of pixels having pixel (x, y) as match in the other view
      float cost = precomputed_disp_match_cost(disparities[i], x, y, cpv);
  ...
  void PatchMatch::disp_perturbation(int x, int y, int cpv, float max_delta_z, float end_dz)
  {
    ...
    // store the initial disparity at location (x, y)
    float initial_disp = disps_[cpv].at<float>(y, x);

    // initializing random number generator (in [end_dz, max_delta_z])
    std::random_device rand_dev;
    std::mt19937 generator(rand_dev());
    std::uniform_int_distribution<int> distr(end_dz, max_delta_z);

    // compute the number of times the disparity will be incremented
    // as a fraction between maximum and minimum delta disparity
    int n_increments = max_delta_z / end_dz;

    // increment at maximum n_increments times the disparity by a
    // factor delta_z
    for (int i = 0; i < n_increments; i++)
    {
      // set delta_z equal to a random number between the
      // maximum and minimum delta disparity
      float delta_z = distr(generator); 

      // increase disparity of delta_z
      float new_disp = disps_[cpv].at<float>(y, x) + delta_z;
      
      // check that the new disparity is not greater than the
      // initial disparity plus the upper bound of delta_z; if it
      // is greater, then break outside the cycle
      if (new_disp > initial_disp + max_delta_z)
      	break;

      // initialize cost with the current matching cost
      // for pixel (x, y); so, if by adding the disparity
      // we go out the image, we won't modify the matching
      // cost of pixel (x, y)
      float new_cost = costs_[cpv].at<float>(y, x);

      if (cpv == 0) 
        // if pixel (x - new_disp, y) is inside the image,
        // then compute its new cost with disparity new_disp
        if (inside(x - new_disp, y, 0, 0, cols_, rows_)) 
          // compute cost for location (x, y) using the new disparity
          new_cost = precomputed_disp_match_cost(new_disp, x, y, cpv);
      else
        // if pixel (x + new_disp, y) is inside the image,
        // then compute its new cost with disparity new_disp
        if (inside(x + new_disp, y, 0, 0, cols_, rows_))
      ...
      // check if the new cost is better that the current lowest cost
      // for pixel (x, y)
      if (new_cost < costs_[cpv].at<float>(y, x))
      {
        // update disparity and cost of location (x, y)
        // if so
